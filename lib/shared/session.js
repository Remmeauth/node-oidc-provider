...
module.exports = async function sessionHandler(ctx, next) {
  const arr = await ctx.oidc.provider.Session.getAll(ctx);
  ctx.oidc.sessions = arr.map((item) => {return new Proxy(item, handler)});
  ctx.oidc.session = new Proxy(await ctx.oidc.provider.Session.get(ctx), handler);
