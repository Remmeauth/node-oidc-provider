...
module.exports = {
  ...
  fromFrontend: [
    ...
    async (ctx, next) => {
      ...
      const { session_items, agent_pub_key } =  ctx.oidc.body;

      const { features: { backchannelLogout } } = instance(ctx.oidc.provider).configuration();

      if (!session_items) throw new InvalidRequest('missing required parameter: session_items');

      for (const profile_id of session_items) {

          const session = await ctx.oidc.provider.Session.find(agent_pub_key, false, profile_id);
          ...
          for (const clientId of clientIds) { // eslint-disable-line no-restricted-syntax
            ...
            if (client) {
              try {
                const { grantId, sid, meta } = session.authorizations[clientId];
                if (backchannelLogout.enabled && client.backchannelLogoutUri) {
                ...
                if (grantId) {
                  if (!session.authorizationFor(clientId).persistsLogout) {
                    await revokeGrant(ctx.oidc.provider, client, grantId);
                    ctx.oidc.provider.emit('grant.revoked', ctx, grantId);
                  } else {
                    const app = await getAdapter(ctx).find(`${session.account}:${clientId}`);
                    if (app) {
                      set(app, `grants[${grantId}]`, session.jti);
                      await getAdapter(ctx).upsert(`${session.account}:${clientId}`, app);
                    }
                  }
                }
