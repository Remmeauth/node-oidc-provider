...
module.exports = {
  ...
  fromBackend: [
    ...
    async (ctx, next) => {
      const { chain_name, items } =  ctx.oidc.body;

      for (const item of items) {
        const session = await ctx.oidc.provider.Session.find(item.session_id);
 
        if (session.authorizations) {
          await Promise.all(
            Object.entries(session.authorizations).map(async ([clientId, { grantId, sid, meta }]) => {
              if ( sid === item.sid && meta.device.account === chain_name ) {
                ...
                delete session.authorizations[clientId];
                await session.save();
                ...
                ctx.oidc.provider.emit('end_sessions.success', {
                  ...
                  client: clientId,
                  sid,
                  chain_name: meta.device.account,
                });
