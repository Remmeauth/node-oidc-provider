    ...
    async (ctx, next) => {
      ...
      for (const item of items) {
        try {
          ...
          const appAdapter  = getAdapter(ctx);
          const app = await appAdapter.find(item) || {};
          const { chainName, grantId } = app;
          ...
          if (chainName === chain_name && client) {
            ...
            // TO DO: del ALL grants
            await appAdapter.destroy(item);
          }          
        } catch (e) {
          continue;
        }
      ...
      for (const item of items) {
        try {
          ...
          if ( sid === authorization_id && session.chainName() === chain_name ) {
            ...
            if ( grantId && !session.authorizationFor(client_id).persistsLogout ) {
              const client = await ctx.oidc.provider.Client.find(client_id).catch(() => {});
