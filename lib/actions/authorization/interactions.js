...
module.exports = async function interactions(resumeRouteName, ctx, next) {
  ...
  if (process.env.SESSION_MODE === 'token') {
    const agent_pub_key = get(oidc, 'verifyedData.sessionId');


    console.log("debag headers:")
    console.log(ctx.req.headers);


    agent = {
      agent_pub_key,
      ip: ctx.req.headers['CF-Connecting-IP'] || ctx.req.headers['x-forwarded-for'],
      metadata: ctx.req.headers['user-agent'],
    }
  }
