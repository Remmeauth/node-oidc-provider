...
async function getInteraction(req, res) {
  ...
  if (process.env.SESSION_MODE === 'token') {
    const splitedUrl = ctx.request.url.split('/');
    id = splitedUrl[2].split('?')[0];
  } else {
    id = ssHandler.get(
      ctx.cookies,
...
class Provider extends events.EventEmitter {
  ...
  async interactionFinished(req, res, result, { mergeWithLastSubmission = true } = {}) {
    ...
    if (process.env.SESSION_MODE === 'token') {
      ...
      res.end('{"success": true, "url": "'+ returnTo +'"}');
    } else {
      res.statusCode = 301; // eslint-disable-line no-param-reassign
